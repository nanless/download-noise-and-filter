...
def get_songs_by_category(
    ...
    filters = [
        ...
        f'duration:[10 TO 180]',
    ]

    fields = "id,name,download,license,previews,tags,username,description,duration,filesize,type,samplerate,bitdepth,channels,created,modified,original_format"
    ...
    # sounds_raw = pages.as_dict()['results']

    # import ipdb; ipdb.set_trace()
    # random.shuffle(sounds_raw)

    # Retrieve sound license information
    with open(os.path.join(basepath, 'details.txt'), 'w') as f:
        f.write("ID,LICENSE,SAMPLE_RATE,TAGS\n")
        f.flush()
        ...
        while True:
            for sound in pages:
                ...
                continue_flag = False
                if sound.samplerate is not None:
                    if sound.samplerate < 16000:
                        continue_flag = True
                for tag in sound.tags:
                    if "speech" in tag.lower() or "voice" in tag.lower() or "people" in tag.lower() or "male" in tag.lower() or "female" in tag.lower():
                        continue_flag = True
                if continue_flag:
                    continue
                ...
                f.write(f"{sound.id},{sound.license},{sound.samplerate},{sound.tags},\n")
                f.flush()
